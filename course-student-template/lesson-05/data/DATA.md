...
本课使用 24 条**合成数据（synthetic data）**，也就是为教学目的由程序生成、并非从真实食堂收集的数据。这些数据只用于练习数据划分、误差计算和事后信息误入模型的反例。一行数据代表“某个取餐窗口在某一天的一次排队记录”。A、B、C 是三个虚构的取餐窗口，每个窗口有 8 天记录。

**特征（feature）**是预测时使用的输入；**标签／目标值（label / target）**是希望预测的真实结果。**数据泄漏（data leakage）**指本不该用于训练、选择或预测的信息进入了这些过程；若输入几乎直接暴露标签，则称为**目标泄漏（target leakage）**。
...
三种方法只重新安排第 1–6 天可用于训练和验证的数据。第 7–8 天始终不进入训练集或验证集。

| 方法 | 训练集 | 验证集 | 主要回答的问题 |
|---|---|---|---|
| `time` 时间顺序划分 | A/B/C 第 1–4 天 | A/B/C 第 5–6 天 | 已有窗口的较晚日期 |
| `group` 分组划分 | 默认 A 第 1–6 天 | 默认 B 第 1–6 天 | 训练中没出现的新窗口 |
| `random` 随机划分 | 从第 1–6 天随机取 12 条 | 其余 6 条 | 没有时间先后或重复对象问题时的一次随机验证 |
...
| `validation_through_day` | 训练和验证数据最晚到第几天 | 固定 `6`，不能包含第 7–8 天测试记录 |
| `group_validation_site` | group 中哪一个第 1–6 天窗口只进入验证集 | `B` 表示 A 训练、B 验证；`A` 表示 B 训练、A 验证 |
...
一行是某种方法对某条验证记录的计算。同一个 `id` 在不同 `method` 下出现，表示同一条原始记录被不同划分方法使用，并不是新增了一条记录。
...
- 由训练集拟合得到的模型参数；“拟合”是用训练数据确定模型参数的过程。

`metrics` 只汇总 `split_strategy` 指定的方法，不表示程序替你选择了最佳方案。`provenance` 是“来源记录”字段，其中的文件和源码摘要由文件字节计算，用于核对重跑时是否使用同一输入。摘要一致不能证明人工数据来自真实环境。
