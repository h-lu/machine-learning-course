...
这是 [本课任务](README.md) 的详细做法，不是额外作业。完成下面的比较、自己的检查和解释，就覆盖必做任务；不用再把 README 的另一条路径重复一遍。你可以使用现成程序和教师帮助，不需要先会写训练代码。**先做第一项小任务：找到 time、group 两行，先数训练与验证样本，不急着比 MAE。**
...
再在 `records.csv` 找 `id=B05`、`method=time`。正常特征是排队时的人数；`leaked_prediction_demo=8` 却是用了事后才得到的小票数 `8.25−0.25`。比较表中的 `leaked_mae_demo=0` 是**数据泄漏反例**，不是可选的优秀模型。分组划分并不能让事后信息变成事前可用信息。
