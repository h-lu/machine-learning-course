...
使用 Python 3.10 以上和仓库已安装的 NumPy 数值计算库。本课只有 24 条人工教学记录，普通 CPU 即可；单次运行通常不到 1 分钟，不需要 GPU、付费模型或联网。人工数据只能解释划分、MAE 和泄漏机制，不能证明真实食堂、真实用户或任何 AI 系统的表现。AI 学习和在线概念检查需要网络，无法连接时按教师安排补做。
