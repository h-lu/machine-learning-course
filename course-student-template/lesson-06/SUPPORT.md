...
这是 [本课任务](README.md) 的详细做法，不是额外作业。完成下面的比较、自己的检查和解释，就覆盖必做任务；不用再把 README 的另一条路径重复一遍。你可以使用现成程序和教师帮助，不需要先会写训练代码。**先做第一项小任务：找到 B05 的回归预测，判断它是低估还是高估。最后要分别判断这个预测适不适合显示分钟数，以及有限提醒该怎样使用。**

## 1. 先知道今天在检查什么

**绝对误差**只看预测相差多少；**非对称损失**对低估、高估赋不同权重。**误报**是提醒了却没有长等，**漏报**是长等却没提醒。先看预测误差，再看提醒决定，不同时修改两类设置。

先读 [LEARN.md](LEARN.md) 的对应例子。术语表按需查，不要为了开始上课把整张表背完。在 `report.md` 先写两个用途：向同学显示预计分钟数；值班人员每轮只能处理少数记录时，优先查看哪些可能长等的记录。分别写低估和高估的后果，并注明默认低估权重 3、容量 2 都是本课的用途假设，不是现实调查结果。其余栏目等拿到结果再填写。
...
| buffered：每条回归预测加 4 分钟 | 3.000 | 3.000 |

缓冲方案没有重新拟合模型，只在每条回归预测上加 4 分钟。它减少了低估，但也可能增加高估。这说明不同指标可以支持不同选择。不要把平均加权误差叫作“实际损失分钟数”，也不要因为缓冲方案在权重 3 下较小，就说它在所有用途里都更好。
...
从 `config-start.json` 另存 `config-mine.json`。根据你设想的人工处理条件，在下面两项中选一个，只改 `capacity`，保持其他设置。

- 改为 `1`：代表只能处理一条提醒。先预计漏报会不会增加，以及这是否会让你暂停使用提醒。
- 改为 `4`：代表最多处理四条提醒。先预计缓冲方案是否会出现误报，以及多出的名额是否一定给了实际长等待记录。

在看结果前写明什么结果会使你保留、限制或暂停方案。再核对 `tp+fp+fn+tn=6`、`alerts≤capacity`，并在 `records.csv` 中列出 linear 和 buffered 各自实际提醒的编号。不要同时改真实长等待定义、提醒阈值或低估权重。
...
先自己数，再用下面数字检查是否读对列：容量 1 时，linear 与 buffered 都只提醒 C06，各有 1 次正确提醒、3 次漏报；容量 4 时，linear 只有 3 条达到阈值，因此提醒 B06、C05、C06，仍漏掉 B05。buffered 有 6 条达到阈值，按预测排序和编号处理 4 条，其中 A06 是误报，B05 仍是漏报。这个结果来自本课人工小数据和固定并列规则，不是普遍结论。

这次填写自己的预计、实际结果和理由，不照抄例子中的采用建议。允许结果支持保留原方案，也允许限制用途或暂不使用。需要 AI 帮助时，可以提问：“请只解释这条命令和这一行数据，先让我计算，再帮我核对；不要替我填最终建议。”

## 6. 写完报告，确认文件已保存

在 `report.md` 回答本课的问题，每项几句话；删除模板提示语，写出配置和结果路径。最少保留：两个用途与预计、一条低估和一条高估的手算、权重 3 与 1 的同预测比较、个人容量检查及实际提醒编号、一项支持证据、一项代价，以及仍不能得出的结论。分别为“显示分钟数”和“有限提醒”写建议。把想保留的参数写回本课 `config.json`，不要改课次字段。
