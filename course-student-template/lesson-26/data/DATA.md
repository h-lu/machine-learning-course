...
`data/base.json` 只含 W01–W12 开发请求；开发模式会拒绝包含 holdout 的文件，也不会生成最终结果。候选设置固定后，显式 final 命令才读取 `data/holdout.json` 的 F01–F06。这个公开教学仓库不能隐藏文件，所以“未见”指它没有参与 S19–S24 的开发运行，也没有在选择候选前查看；如果提前打开或运行过，就应把它改称开发证据并另找新数据。
