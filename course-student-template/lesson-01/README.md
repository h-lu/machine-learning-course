...
不要只把斜率改成另一个数字。请从 8 条训练记录中选两条不同人数的记录，用两点式写出一条规则 `预测 = 截距 + 斜率 × 人数`，并确认得到的参数至少有一个不同于默认的截距或斜率；如果恰好相同，换一对记录或解释为什么。再把这条规则写入配置并运行。把 8 条记录的预测、实际值和绝对误差整理成表，与默认规则比较 MAE；说明你的两条记录为什么会让规则在其他人数上变好或变坏。报告必须同时保留两条选用记录、参数算式、8 条误差和一个超出 0–3 人范围的检查。
