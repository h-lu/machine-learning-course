...
每条记录还包含 `staff_count`、`service_mode` 和 `rain`。它们记录可能影响等待的现场条件，但默认的三种方法只使用 `queue_length`；本课比较的是“只看人数”的办法。你可以在报告中检查误差是否集中在某种背景条件，但不要声称程序已经使用了这些字段，除非你实际修改代码并保存了新结果。
