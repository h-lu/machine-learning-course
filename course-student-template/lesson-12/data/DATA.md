...
基于同一种浅树比较单树、Bootstrap集成和梯度提升的误差。
...
可调整参数（见 `../config.json`）：`seed`=7, `n_estimators`=8, `max_depth`=2, `learning_rate`=0.2。
...
| `n_estimators` | 集成中实际拟合的浅树数量。 |
| `max_depth` | 一棵树最多连续分裂几层。 |
| `learning_rate` | 每轮梯度提升加入新树预测时的缩减比例；本例拟合平方误差残差。 |
