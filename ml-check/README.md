...
接口保留 `GET /ml-check/healthz`、`GET /ml-check/api/lessons` 和 `GET /ml-check/api/lessons/{lesson_id}`。这是本地只读服务，没有学生登录、答题存储、自动评分或生产部署。不要把它描述成已经上线的学习平台。
...
旧服务依赖 FastAPI，新本地读取服务使用标准库；启动命令已改变。题目响应改为明确的 A/B 题目列表，默认不含答案。旧代码与完整配置在总工作区的归档中，远程服务未被改动。
