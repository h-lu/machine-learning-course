...
32 课各有 A、B 两阶段，每阶段五题，共 320 题。每题使用 `C01-A-01`、`S01-B-05` 这样的稳定编号，页面标题也直接显示课次编号；先完成 A 版判断，再只围绕 A 版内容进行 AI 学习，最后由教师开放 B 版换场景检验。题库版本为 `ml-v3-2026-09-12`。题目答案与解释保留在教师/服务文件中；学生读取接口只返回题干与选项。
