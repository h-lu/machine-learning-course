...
本分支源码题库为 `ml-v10-s01-review-2026-09-19`，在入门候选基础上重写 S01–S06；C01–S06 的短阶段时长与新课包一起采用。页面运行器仍按课号读取单份当前题库，不会自动隔离旧课内容。启用前按教师模块发布记录准备独立实例和数据库、备份、版本核对及恢复；本次开发不执行本页的生产更新命令。

S07 概念题也已按同一知识点配对修订，仍保持原阶段时长。切换时必须保留旧题和旧场次的对应关系；不能用当前源码覆盖旧场次的解释。

当前 v10 相较 v9 仅修订第三课 S01；题库其余 31 课及所有时长保持不变。单课文字更新也不能原地覆盖旧场次的题目解释。
