...
本分支源码题库为 `ml-v9-guided-review-2026-09-19`，在入门候选基础上重写 S01–S06；C01–S06 的短阶段时长与新课包一起采用。页面运行器仍按课号读取单份当前题库，不会自动隔离旧课内容。启用前按教师模块发布记录准备独立实例和数据库、备份、版本核对及恢复；本次开发不执行本页的生产更新命令。
